...
// #define BLYNK_TEMPLATE_ID           "TMPL6-mxPQLxt"
// #define BLYNK_TEMPLATE_NAME         "Quickstart Device"
// #define BLYNK_AUTH_TOKEN            "gq6fU0TKbrFtwIUcppVQedYP25N2PCm9"
...
void sendNotification(){
  ...
  LOG_ALT("Motion Detected");

  // Get the current time
  String currentTime = getCurrentTime(); // Assuming getCurrentTime() is a function that returns the current time as a String
  // Iterate over each character in the string
  for (size_t i = 0; i < currentTime.length(); ++i) {
  ...
  //Enter you own api key of whatabot or callmebot
  
  String apiUrl = "https://api.whatabot.net/whatsapp/sendMessage?apikey=";
  String apiUrl2 = "https://api.callmebot.com/whatsapp.php";
  ...
  // Send HTTP GET request and get response code
  ...
  if (httpResponseCode == HTTP_CODE_OK) {
    ...
    Serial.print("Failed to send notification. Error code: ");
    Serial.println(httpResponseCode);
    success = false;
    
  }
  if (!success){
    Serial.print(apiUrl2);
    http.begin(apiUrl2);
